...
Script Purposes
	  ...
	  propulate the 'silver' schema tables from the 'bronze' schema.
Actions Performed:
...
BEGIN
	...
	SELECT 
		...
		ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE) -- Chnaging datatype from int to date. You have to go to Varchar 1st in SQL
		END AS sls_order_dt,
		CASE WHEN sls_ship_dt = 0 OR LEN (sls_ship_dt) !=8 THEN NULL
		ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE) -- Chnaging datatype from int to date. You have to go to Varchar 1st in SQL
		END AS sls_ship_dt,
		CASE WHEN sls_due_dt = 0 OR LEN (sls_due_dt) !=8 THEN NULL
		ELSE CAST(CAST(sls_due_dt AS VARCHAR) AS DATE) -- Chnaging datatype from int to date. You have to go to Varchar 1st in SQL
		END AS sls_due_dt,
